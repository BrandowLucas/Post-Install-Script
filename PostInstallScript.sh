...
sourcegit zed github-cli git-credential-manager recaf gdb cppcheck strace ltrace
...
shell_config=(
    ...
    'alias topmem='\''ps -eo rss,pid,user,command | sh -c "read -r h; echo \"\$h\"; sort -k1,1 -nr" | head -n 11 | numfmt --header --field=1 --from-unit=K --to=iec | awk "NR==1 {print \"\033[1;37m\" \$0 \"\033[0m\"} NR>1 {printf \"\033[1;32m%8s\033[0m \033[1;34m%-6s\033[0m \033[1;33m%-8s\033[0m %s\n\", \$1, \$2, \$3, \$4}"'\'''
    'alias killtop='\''kill -9 $(ps -eo rss,pid | sort -k1,1 -nr | head -n 1 | awk "{print \$2}")'\'''
    'alias topdisk='\''python3 -c '\''\''exec("import os,glob,time,pwd\ndef get_io():\n d={}\n for p in glob.glob(\"/proc/[0-9]*\"):\n  try:\n   with open(f\"{p}/io\") as f: d[os.path.basename(p)]=(int(dict(l.strip().split(\": \") for l in f if \":\" in l).get(\"read_bytes\",0)),int(dict(l.strip().split(\": \") for l in f if \":\" in l).get(\"write_bytes\",0)))\n  except Exception: pass\n return d\nt1,s1=time.time(),get_io()\ntime.sleep(1.0)\nt2,s2=time.time(),get_io()\ndt=t2-t1\ndef fmt(b):\n if b>=1024**3: return f\"{b/1024**3:.1f}G/s\"\n if b>=1024**2: return f\"{b/1024**2:.1f}M/s\"\n if b>=1024: return f\"{b/1024:.0f}K/s\"\n if b>0: return f\"{b:.0f}B/s\"\n return \"0B/s\"\nprocs=[]\nfor pid,(r2,w2) in s2.items():\n if pid in s1:\n  dr,dw=max(0.0,(r2-s1[pid][0])/dt),max(0.0,(w2-s1[pid][1])/dt)\n  if dr+dw>0:\n   cmd=\"\"\n   try:\n    with open(f\"/proc/{pid}/cmdline\",\"rb\") as f: cmd=f.read().split(bytes([0]))[0].decode(\"utf-8\",\"ignore\").split()[0]\n   except Exception: pass\n   if not cmd:\n    try:\n     with open(f\"/proc/{pid}/comm\",\"r\") as f: cmd=f.read().strip()\n    except Exception: pass\n   user=\"?\"\n   try: user=pwd.getpwuid(os.stat(f\"/proc/{pid}\").st_uid).pw_name\n   except Exception: pass\n   procs.append((dr+dw,dr,dw,pid,user,cmd))\nprocs.sort(reverse=True)\nprint(\"\\x1b[1;37m    READ/s   WRITE/s     PID USER     COMMAND\\x1b[0m\")\nif not procs: print(\"    No active disk I/O detected.\")\nfor tot,dr,dw,pid,user,cmd in procs[:10]:\n print(f\"\\x1b[1;32m{fmt(dr):>10}\\x1b[0m \\x1b[1;36m{fmt(dw):>9}\\x1b[0m \\x1b[1;34m{pid:>7}\\x1b[0m \\x1b[1;33m{user:<8}\\x1b[0m {cmd}\")\n")'\'\'''
    'alias topdisktotal='\''python3 -c '\''\''exec("import os,glob,pwd\ndef fmt(b):\n if b>=1024**3: return f\"{b/1024**3:.1f}G\"\n if b>=1024**2: return f\"{b/1024**2:.1f}M\"\n if b>=1024: return f\"{b/1024:.0f}K\"\n if b>0: return f\"{b:.0f}B\"\n return \"0B\"\nprocs=[]\nfor p in glob.glob(\"/proc/[0-9]*\"):\n try:\n  pid=os.path.basename(p)\n  with open(f\"{p}/io\",\"r\") as f: io=dict(l.strip().split(\": \") for l in f if \":\" in l)\n  r,w=int(io.get(\"read_bytes\",0)),int(io.get(\"write_bytes\",0))\n  if r+w==0: continue\n  cmd=\"\"\n  try:\n   with open(f\"{p}/cmdline\",\"rb\") as f: cmd=f.read().split(bytes([0]))[0].decode(\"utf-8\",\"ignore\").split()[0]\n  except Exception: pass\n  if not cmd:\n   try:\n    with open(f\"{p}/comm\",\"r\") as f: cmd=f.read().strip()\n   except Exception: pass\n  user=\"?\"\n  try: user=pwd.getpwuid(os.stat(f\"{p}\").st_uid).pw_name\n  except Exception: pass\n  procs.append((r+w,r,w,pid,user,cmd))\n except Exception: pass\nprocs.sort(reverse=True)\nprint(\"\\x1b[1;37m      READ     WRITE     PID USER     COMMAND\\x1b[0m\")\nif not procs: print(\"    No disk I/O data available.\")\nfor tot,r,w,pid,user,cmd in procs[:10]:\n print(f\"\\x1b[1;32m{fmt(r):>10}\\x1b[0m \\x1b[1;36m{fmt(w):>9}\\x1b[0m \\x1b[1;34m{pid:>7}\\x1b[0m \\x1b[1;33m{user:<8}\\x1b[0m {cmd}\")\n")'\'\'''
    ...
    'alias newip_warp="warp-cli registration new"'
)
